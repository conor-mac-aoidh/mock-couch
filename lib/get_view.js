...
module.exports = function (self) {
  ...
  return function (req, res, next) {
    ...
    // db/design doc not found
    if (!self.databases[req.params.db] || !self.databases[req.params.db]['_design/' + req.params.doc]) {
      res.send(404, {
        error: 'not found',
        reason: 'missing'
      });
    ...
    // missing named view
    if (!self.databases[req.params.db]['_design/' + req.params.doc].views[req.params.name]) {
      res.send(404, {
        error: 'not found',
